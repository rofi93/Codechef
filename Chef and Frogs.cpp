...
bool operator<(struct frog A, struct frog B)
{
    return A.value<B.value;
}
int main()
{
    LL n,k,p,i,x,y,u,v,j;
    while(cin>>n>>k>>p)
    {
        ...
        for(i=1;i<n;i++)
        {
            if(a[i].value-a[i-1].value<=k)
            {
                objects.union_set(a[i].index,a[i-1].index);
            }
        }
        for(i=1; i<=p; i++)
        {
            scanf("%lld %lld",&x,&y);
            u=objects.find_set(x);
            v=objects.find_set(y);
            if(u==v)
                printf("Yes\n");
            else
                printf("No\n");
